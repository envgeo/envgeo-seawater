...
ローカルで繰り返し使う場合は、研究者自身のデータを
`local_data/user_data.xlsx` に置くか、`ENVGEO_LOCAL_USER_DATA_PATH` で
CSV、XLSX、XLSを指定すると自動的に読み込みます。ローカルデータは
「ローカルユーザーの常時読み込み用表」として扱い、`User Excel data` という
データセット名を付けて、選択した各参照データに結合します。ブラウザで
毎回アップロードしなくても、アプリ起動時からData filteringで選択できます。
`local_data/` はGit管理対象外です。
ローカル表を置き換えたり編集した後は、Streamlitを再起動するかデータキャッシュを
クリアして、更新したブックを再読み込みしてください。
